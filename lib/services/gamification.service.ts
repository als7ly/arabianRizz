...
export async function updateGamification(userId: string) {
  try {
    ...
    // 2. Update Streak
    const now = new Date();
    // Handle potential missing streak object or legacy data
    const lastActive = user.streak?.lastActive ? new Date(user.streak.lastActive) : new Date(0);
    ...
    if (!user.streak) {
        user.streak = { current: 1, lastActive: now };
    }
